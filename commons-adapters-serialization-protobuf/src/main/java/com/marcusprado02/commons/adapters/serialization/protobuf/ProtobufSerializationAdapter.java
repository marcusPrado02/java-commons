...
public class ProtobufSerializationAdapter<T extends Message> implements SerializationPort<T> {
   ...
   */
  public ProtobufSerializationAdapter(
      Class<T> messageClass, SchemaRegistry schemaRegistry, SerializationOptions defaultOptions) {
    ...
    try {
      ...
      if (options.isValidateSchema() && schemaRegistry != null) {
        ...
        if (!validationResult.isOk() || !validationResult.getOrNull().isValid()) {
          return Result.fail(
              Problem.of(
                  ErrorCode.of("UNKNOWN"),
                  ErrorCategory.BUSINESS,
                  Severity.ERROR,
                  "Error: Object does not conform to schema"));
        }
      }
  ...
  public Result<T> deserialize(byte[] data, Class<T> targetClass, SerializationOptions options) {
    try {
      ...
      return Result.fail(
          Problem.of(
              ErrorCode.of("DESERIALIZATION_ERROR"),
              ErrorCategory.BUSINESS,
              Severity.ERROR,
  ...
  private Result<byte[]> serializeToJson(T object) {
    try {
      ...
      return Result.fail(
          Problem.of(
              ErrorCode.of("JSON_SERIALIZATION_ERROR"),
              ErrorCategory.BUSINESS,
              Severity.ERROR,
  ...
  private Result<byte[]> serializeToText(T object) {
    try {
      ...
      return Result.fail(
          Problem.of(
              ErrorCode.of("TEXT_SERIALIZATION_ERROR"),
              ErrorCategory.BUSINESS,
              Severity.ERROR,
  ...
  private Result<ValidationResult> validateWithRegistry(T object, SerializationOptions options) {
    try {
      ...
      return Result.fail(
          Problem.of(
              ErrorCode.of("REGISTRY_VALIDATION_ERROR"),
              ErrorCategory.BUSINESS,
              Severity.ERROR,
