            ...
            for (SearchFilter filter : criteria.filters()) {
                switch (filter.operator()) {
                    case EQ -> predicates.add(
                        builder.equal(root.get(filter.field()), filter.value())
                    );
                    case LIKE -> predicates.add(
                        builder.like(root.get(filter.field()), filter.value())
                    );
                    case GT -> predicates.add(
                        builder.greaterThan(root.get(filter.field()), filter.value())
                    );
                    case LT -> predicates.add(
                        builder.lessThan(root.get(filter.field()), filter.value())
                    );
                    case GTE -> predicates.add(
                        builder.greaterThanOrEqualTo(root.get(filter.field()), filter.value())
                    );
                    case LTE -> predicates.add(
                        builder.lessThanOrEqualTo(root.get(filter.field()), filter.value())
                    );
                    case NEQ -> predicates.add(
                        builder.notEqual(root.get(filter.field()), filter.value())
                    );
                    case IN -> {
