...
public final class SpringHttpResultMapperWithContext
    implements HttpResultMapperWithContext {
  ...
  public <T> HttpResultResponseWithContext mapOk(
    ...
    ApiEnvelopeWithContext.Success<T> body =
        ApiEnvelopeWithContext.Success.of(
            ok.value(), correlationId, tenantId, actorId);
  ...
  public HttpResultResponseWithContext mapProblem(
    ...
    int status = switch (problem.category()) {
      case VALIDATION -> 400;
      case BUSINESS -> 422;
      case NOT_FOUND -> 404;
      case CONFLICT -> 409;
