...
/** Benchmarks for repository operations. Tests performance of CRUD operations and queries. */
...
public class RepositoryBenchmark {
  ...
  public TestEntity saveOperation() {
    TestEntity newEntity = new TestEntity(UUID.randomUUID().toString(), "New Entity", 999);
  ...
  public static class TestRepository extends InMemoryPageableRepository<TestEntity, String> {
    ...
    public List<TestEntity> findByName(String name) {
      return findAll().stream().filter(e -> e.getName().equals(name)).toList();
    }

    public List<TestEntity> findByGroup(int group) {
      return findAll().stream().filter(e -> e.getGroup() == group).toList();
    }
  }
}
