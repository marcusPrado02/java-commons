...
public class RateLimiterBenchmark {
  ...
  public void setup() {
    ...
    unlimitedLimiter =
        RateLimiterFactory.inMemory()
            .withConfig(RateLimitConfig.perSecond(1_000_000))
            .build();

    // Restricted limiter — most requests will be rejected (1 req/sec)
    restrictedLimiter =
        RateLimiterFactory.inMemory()
            .withConfig(RateLimitConfig.perSecond(1))
            .build();
  ...
  /**
   * Benchmark: tryConsume on a single key with a high-capacity limiter.
   * Models the happy path — nearly all tokens are available.
   */
  @Benchmark
  ...
  /**
   * Benchmark: tryConsume with fan-out across many distinct keys.
   * Each call uses a different key (round-robin over 1000 keys),
   * simulating per-user rate limiting at scale.
   */
  @Benchmark
  ...
  /**
   * Benchmark: tryConsume on a heavily throttled limiter.
   * Measures rejection overhead when the bucket is exhausted.
   */
  @Benchmark
  ...
  /**
   * Benchmark: tryConsume with burst consumption (2 tokens at once).
   * Measures cost of multi-token requests.
   */
  @Benchmark
