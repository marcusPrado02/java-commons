 ...
 *
 * <p>These tests ensure the rules in {@link SolidRules} are green before they can
 * be promoted to mandatory build gates in consuming projects.
 *
 * <p>Rules are checked against the commons kernel and ports — the layers where SOLID
 * violations would be most damaging. Adapter modules are excluded from most checks
 * because they intentionally depend on third-party frameworks.
 */
class SolidRulesArchTest {
  ...
  static void importClasses() {
    var importer = new ClassFileImporter().withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_TESTS);
    KERNEL_CLASSES =
        importer.importPackages(
            "com.marcusprado02.commons.kernel",
            "com.marcusprado02.commons.ports");
    ALL_CLASSES =
        importer.importPackages("com.marcusprado02.commons");
  }
  ...
  void srp_domain_classes_must_not_mix_http_or_persistence_concerns() {
    SolidRules.srpDomainClassesShouldNotMixConcerns()
        .allowEmptyShould(true)
  ...
  void ocp_adapters_must_implement_interfaces() {
    ...
    SolidRules.ocpAdaptersShouldImplementInterfaces()
        .allowEmptyShould(true)
        .check(ALL_CLASSES);
  }
  ...
  void dip_application_must_not_depend_on_adapters() {
    ...
    SolidRules.dipApplicationShouldNotDependOnAdapters()
        .allowEmptyShould(true)
        .check(ALL_CLASSES);
  }
}
