...
public final class Resilience4jExecutor implements ResilienceExecutor {
    ...
    public <T> T supply(String name, ResiliencePolicySet policies, Supplier<T> action) {
        ...
        ResiliencePolicySet safePolicies = (policies == null) ? new ResiliencePolicySet(null, null, null, null) : policies;

        Supplier<T> decorated = Decorators.ofSupplier(action)
    ...
    private TimeLimiter buildTimeLimiter(String name, ResiliencePolicySet policies) {
        ...
        TimeLimiterConfig cfg = TimeLimiterConfig.custom()
                .timeoutDuration(policies.timeout().timeout())
                .build();
