...
class AzureBlobFileStoreAdapterTest {
  ...
  private static final GenericContainer<?> azuriteContainer =
  ...
  void setUp() {
    String connectionString =
        String.format(
            ...
            azuriteContainer.getHost(), azuriteContainer.getMappedPort(AZURITE_BLOB_PORT));
  ...
  void shouldUploadFileWithMetadata() {
    ...
    FileStorePort.UploadOptions options =
        FileStorePort.UploadOptions.builder().contentType("text/plain").metadata(metadata).build();
  ...
  void shouldGetFileMetadata() {
    ...
    FileStorePort.UploadOptions options =
        FileStorePort.UploadOptions.builder().contentType(contentType).build();
  ...
  void shouldListFilesWithPrefix() {
    ...
    adapter.upload(
        new FileId(CONTAINER_NAME, "prefix/file1.txt"),
        new ByteArrayInputStream("content1".getBytes()),
        FileStorePort.UploadOptions.builder().build());
    adapter.upload(
        new FileId(CONTAINER_NAME, "prefix/file2.txt"),
        new ByteArrayInputStream("content2".getBytes()),
        FileStorePort.UploadOptions.builder().build());
    adapter.upload(
        new FileId(CONTAINER_NAME, "other/file3.txt"),
    ...
    assertThat(result.getOrNull().files())
  ...
  void shouldGeneratePresignedUrlForPut() {
    ...
    Result<URL> result =
        adapter.generatePresignedUrl(
            fileId, FileStorePort.PresignedOperation.PUT, Duration.ofHours(1));
  ...
  void shouldUploadWithStorageClass() {
    ...
    Result<FileStorePort.UploadResult> result =
        adapter.upload(fileId, new ByteArrayInputStream(content.getBytes()), options);
