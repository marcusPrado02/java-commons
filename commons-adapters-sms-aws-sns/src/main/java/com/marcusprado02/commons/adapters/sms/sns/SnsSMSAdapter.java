 ...
 *
 * <p>This adapter uses Amazon Simple Notification Service (SNS) to send SMS messages. It supports
...
public class SnsSMSAdapter implements SMSPort, AutoCloseable {
  ...
  public Result<Void> verify() {
    try {
      // Test connection by checking if client is configured (SNS doesn't have a lightweight test
      // operation)
  ...
  private SnsClient createSnsClient(SnsConfiguration config) {
    ...
    var builder = SnsClient.builder().region(config.region()).overrideConfiguration(clientConfig);
  ...
  private PublishRequest buildPublishRequest(SMS sms, SMSOptions options) {
    ...
    if (configuration.deliveryStatusLogging()) {
      smsAttributes.put(
          "AWS.SNS.SMS.DeliveryStatusLogging",
          MessageAttributeValue.builder().stringValue("true").dataType("String").build());
    }
  ...
  private Problem mapSnsException(Exception e) {
    return switch (e) {
      ...
      case AuthorizationErrorException ex ->
          Problem.of(
      ...
      case InvalidParameterValueException ex ->
          Problem.of(
