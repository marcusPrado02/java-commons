...
class SnsConfigurationTest {
  ...
  void shouldCreateDevelopmentConfiguration() {
    SnsConfiguration config =
        SnsConfiguration.forDevelopment(
            Region.US_WEST_2, "AKIAIOSFODNN7EXAMPLE", "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY");
  ...
  void shouldCreateProductionConfiguration() {
    SnsConfiguration config =
        SnsConfiguration.forProduction(
            Region.US_EAST_1,
  ...
  void shouldFailWhenNoCredentialsProvided() {
    IllegalArgumentException exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> SnsConfiguration.builder().region(Region.US_EAST_1).build());
        ...
        assertThrows(
            ...
            () ->
                SnsConfiguration.builder()
                    ...
                    .build());
    assertEquals(
        "Secret access key is required when access key ID is provided", exception.getMessage());
  }
  ...
  void shouldAllowBlankAccessKeyWithSessionToken() {
    assertDoesNotThrow(
        () ->
            SnsConfiguration.builder().region(Region.US_EAST_1).sessionToken("session123").build());
  }
