...
/** AWS S3 implementation of FileStorePort. Provides file storage operations using Amazon S3. */
public class S3FileStoreAdapter implements FileStorePort {
  ...
  public Result<FileObject> download(FileId fileId) {
    try {
      ...
      GetObjectRequest request =
          GetObjectRequest.builder().bucket(fileId.bucket()).key(fileId.key()).build();
  ...
  public Result<Void> delete(FileId fileId) {
    try {
      ...
      DeleteObjectRequest request =
          DeleteObjectRequest.builder().bucket(fileId.bucket()).key(fileId.key()).build();
  ...
  public Result<DeleteResult> deleteAll(List<FileId> fileIds) {
    try {
      ...
      for (FileId fileId : fileIds) {
        bucketGroups
            .computeIfAbsent(fileId.bucket(), k -> new ArrayList<>())
  ...
  public Result<Boolean> exists(FileId fileId) {
    try {
      ...
      HeadObjectRequest request =
          HeadObjectRequest.builder().bucket(fileId.bucket()).key(fileId.key()).build();
  ...
  public Result<ListResult> list(String bucket, String prefix, ListOptions options) {
    try {
      ...
      ListObjectsV2Request.Builder requestBuilder =
          ListObjectsV2Request.builder().bucket(bucket).prefix(prefix).maxKeys(options.maxKeys());
      ...
      List<FileId> files =
          response.contents().stream().map(s3Object -> new FileId(bucket, s3Object.key())).toList();

      log.info("Listed {} files in bucket: {}", files.size(), bucket);

      return Result.ok(
          new ListResult(files, response.nextContinuationToken(), response.isTruncated()));
  ...
  public Result<URL> generatePresignedUrl(
    ...
    try {
      log.debug(
          "Generating presigned URL for file: {}, operation: {}, duration: {}",
          fileId,
          operation,
          duration);
          ...
          switch (operation) {
            ...
            case PUT -> {
              PutObjectRequest putRequest =
                  PutObjectRequest.builder().bucket(fileId.bucket()).key(fileId.key()).build();
