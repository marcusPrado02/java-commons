 ...
 *
 * <p>This record provides a type-safe way to configure compression parameters such as algorithm,
 * compression level, buffer size, and other algorithm-specific settings.
 *
...
public record CompressionOptions(
  ...
  public CompressionOptions {
    ...
    if (level < minLevel || level > maxLevel) {
      throw new IllegalArgumentException(
          String.format(
              "Compression level %d is not valid for %s (valid range: %d-%d)",
  ...
  public static CompressionOptions fastFor(CompressionAlgorithm algorithm) {
    return builder(algorithm).level(getMinCompressionLevel(algorithm)).build();
  }
  ...
  public static CompressionOptions bestFor(CompressionAlgorithm algorithm) {
    return builder(algorithm).level(getMaxCompressionLevel(algorithm)).build();
  }
  ...
  public static class Builder {
    ...
    public CompressionOptions build() {
      return new CompressionOptions(
          algorithm, level, bufferSize, headerEnabled, checksumEnabled, windowSize);
    }
  }
}
