 ...
 *
 * <ul>
 *   <li><b>GZIP</b>: Good balance between speed and compression ratio, widely supported
 *   <li><b>DEFLATE</b>: Similar to GZIP but without headers, used in ZIP files
 *   <li><b>BROTLI</b>: Superior compression ratio, especially for text/HTML, slower compression
 *   <li><b>LZ4</b>: Extremely fast compression/decompression, lower compression ratio
 *   <li><b>ZSTD</b>: Great balance of speed and compression, Facebook's Zstandard
 *   <li><b>SNAPPY</b>: Fast compression focused on speed over ratio, used by Google
 * </ul>
 *
 * @author Marcus Prado
 * @since 1.0.0
...
public enum CompressionAlgorithm {
  ...
  /**
   * GZIP compression algorithm (RFC 1952). Good balance between compression ratio and speed. Widely
   * supported across platforms and libraries.
   */
  GZIP("gzip", "application/gzip", new String[] {"gz", "gzip"}),

  /**
   * DEFLATE compression algorithm (RFC 1951). Similar to GZIP but without the header and checksum.
   * Used in ZIP files and HTTP compression.
   */
  DEFLATE("deflate", "application/octet-stream", new String[] {"deflate"}),

  /**
   * Brotli compression algorithm (RFC 7932). Superior compression ratio especially for text
   * content. Supported by modern browsers for HTTP compression.
   */
  BROTLI("br", "application/brotli", new String[] {"br", "brotli"}),

  /**
   * LZ4 compression algorithm. Extremely fast compression and decompression. Good choice when speed
   * is more important than compression ratio.
   */
  LZ4("lz4", "application/lz4", new String[] {"lz4"}),

  /**
   * Zstandard (ZSTD) compression algorithm. Great balance between speed and compression ratio.
   * Developed by Facebook, gaining widespread adoption.
   */
  ZSTD("zstd", "application/zstd", new String[] {"zst", "zstd"}),

  /**
   * Snappy compression algorithm. Focuses on speed rather than compression ratio. Developed by
   * Google, used in BigTable and other systems.
   */
