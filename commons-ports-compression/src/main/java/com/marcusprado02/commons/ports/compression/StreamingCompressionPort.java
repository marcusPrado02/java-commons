 ...
 *
 * <p>This interface provides streaming compression capabilities that allow processing of large
 * datasets without loading everything into memory. It supports both push and pull streaming
 * patterns.
 *
 * @author Marcus Prado
 * @since 1.0.0
...
public interface StreamingCompressionPort {
  ...
  interface StreamingCompressor extends AutoCloseable {
    ...
    /**
     * Finishes compression and writes any remaining data. Must be called before closing.
     *
    ...
    /**
     * Closes the compressor and releases resources. Automatically calls finish() if not already
     * called.
     */
    @Override
    void close();
