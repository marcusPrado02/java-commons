 ...
 *
 * <p>This class provides utilities for working with HTTP compression headers,
 * content negotiation, and compression algorithm selection based on client preferences.
 *
 * @author Marcus Prado
 * @since 1.0.0
 */
public final class HttpCompression {
     ...
     * Text-based content types generally benefit from compression.
     *
    ...
    public static boolean isCompressible(String mimeType) {
        ...
        return normalized.equals("application/json") ||
               ...
               normalized.startsWith("application/json") || // includes application/json; charset=utf-8
    ...
    public static CompressionOptions createHttpOptions(CompressionAlgorithm algorithm, boolean fast) {
        CompressionOptions.Builder builder = CompressionOptions.builder(algorithm)
                .bufferSize(8192); // Smaller buffer for HTTP streams

        if (fast) {
            return CompressionOptions.fastFor(algorithm);
        } else {
            // Balanced compression for HTTP (not maximum to avoid high CPU usage)
            int defaultLevel = switch (algorithm) {
                case GZIP, DEFLATE -> 6;
                case BROTLI -> 4;
                case LZ4 -> 1;
                case SNAPPY -> 1;
