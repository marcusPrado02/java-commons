 ...
 *
 * <p>This factory provides convenient methods for creating compression adapters
 * with different performance profiles and algorithm preferences.
 *
 * @author Marcus Prado
 * @since 1.0.0
 */
public final class CompressionFactory {
    ...
    /**
     * Creates a compression configuration optimized for speed.
     * Uses fast algorithms with minimal compression levels.
     *
    ...
    /**
     * Creates a compression configuration optimized for compression ratio.
     * Uses algorithms that provide best compression ratios with higher levels.
     *
    ...
    /**
     * Creates a compression configuration balanced between speed and ratio.
     * Uses moderate compression levels with versatile algorithms.
     *
    ...
    /**
     * Creates a compression configuration for web/HTTP usage.
     * Uses algorithms commonly supported by web browsers and proxies.
     *
    ...
    public static class CompressionConfiguration {
        ...
        public CompressionOptions createOptions(CompressionAlgorithm algorithm) {
            ...
            if (preferSpeed) {
                ...
                level = Math.min(maxCompressionLevel, switch (algorithm) {
                    case GZIP, DEFLATE -> 9;
                    case BROTLI -> 11;
                    case LZ4 -> 12;
                    case SNAPPY -> 1;
        ...
        public static class Builder {
            ...
            public Builder defaultAlgorithm(CompressionAlgorithm algorithm) {
                this.defaultAlgorithm = Objects.requireNonNull(algorithm, "Default algorithm cannot be null");
                return this;
            }

            public Builder fallbackAlgorithm(CompressionAlgorithm algorithm) {
                this.fallbackAlgorithm = Objects.requireNonNull(algorithm, "Fallback algorithm cannot be null");
                return this;
            }
    ...
    public static class AdaptiveCompressionPort implements CompressionPort {
        ...
        @Override
        public com.marcusprado02.commons.kernel.result.Result<com.marcusprado02.commons.ports.compression.CompressionResult> compress(
        ...
        @Override
        public com.marcusprado02.commons.kernel.result.Result<com.marcusprado02.commons.ports.compression.CompressionResult> decompress(
                java.io.InputStream input, java.io.OutputStream output, CompressionOptions options) {
            return delegate.decompress(input, output, options);
        }

        @Override
        public com.marcusprado02.commons.kernel.result.Result<byte[]> compress(byte[] data, CompressionOptions options) {
            return delegate.compress(data, options);
        }

        @Override
        public com.marcusprado02.commons.kernel.result.Result<byte[]> decompress(byte[] compressedData, CompressionOptions options) {
            return delegate.decompress(compressedData, options);
        }

        @Override
        public com.marcusprado02.commons.kernel.result.Result<Double> getCompressionRatio(byte[] data, CompressionOptions options) {
            return delegate.getCompressionRatio(data, options);
        }

        @Override
        public com.marcusprado02.commons.kernel.result.Result<Boolean> isCompressed(byte[] data, CompressionAlgorithm algorithm) {
