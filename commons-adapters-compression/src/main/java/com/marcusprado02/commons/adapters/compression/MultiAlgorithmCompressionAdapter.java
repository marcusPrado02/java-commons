...
public class MultiAlgorithmCompressionAdapter implements CompressionPort {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(MultiAlgorithmCompressionAdapter.class);

  private final boolean brotliAvailable;
  private final LZ4Factory lz4Factory;

  /**
   * Creates a new multi-algorithm compression adapter. Initializes all compression libraries and
   * logs availability.
   */
  ...
  @Override
  public Result<CompressionResult> compress(
  ...
  public Result<Double> getCompressionRatio(byte[] data, CompressionOptions options) {
    ...
    if (!compressed.isOk()) {
      return Result.fail(compressed.problemOrNull());
    }
    ...
    double ratio = (double) compressed.getOrNull().length / data.length;
  ...
  private Result<CompressionResult> compressGzip(
  ...
  private Result<CompressionResult> decompressGzip(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long compressedSize = 0;
    long decompressedSize = 0;
  ...
  private Result<CompressionResult> compressDeflate(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long originalSize = 0;
    long compressedSize = 0;
  ...
  private Result<CompressionResult> decompressDeflate(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long compressedSize = 0;
    long decompressedSize = 0;
  ...
  private Result<CompressionResult> compressBrotli(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long originalSize = 0;
    long compressedSize = 0;
    ...
    return Result.ok(
        CompressionResult.builder(CompressionAlgorithm.BROTLI)
            .originalSize(originalSize)
            .compressedSize(compressedSize)
            .build());
  }

  private Result<CompressionResult> decompressBrotli(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long compressedSize = 0;
    long decompressedSize = 0;
  ...
  private Result<CompressionResult> compressLZ4(
    ...
    try (LZ4BlockOutputStream lz4Out =
        new LZ4BlockOutputStream(output, options.bufferSize(), compressor)) {
      byte[] buffer = new byte[options.bufferSize()];
      int bytesRead;
      while ((bytesRead = input.read(buffer)) != -1) {
  ...
  private Result<CompressionResult> compressZstd(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long originalSize = 0;
    long compressedSize = 0;
  ...
  private Result<CompressionResult> decompressZstd(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long compressedSize = 0;
    long decompressedSize = 0;
  ...
  private Result<CompressionResult> compressSnappy(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long originalSize = 0;
    long compressedSize = 0;
    ...
    return Result.ok(
        CompressionResult.builder(CompressionAlgorithm.SNAPPY)
            .originalSize(originalSize)
            .compressedSize(compressedSize)
            .build());
  }

  private Result<CompressionResult> decompressSnappy(
      InputStream input, OutputStream output, CompressionOptions options) throws IOException {
    long compressedSize = 0;
    long decompressedSize = 0;
