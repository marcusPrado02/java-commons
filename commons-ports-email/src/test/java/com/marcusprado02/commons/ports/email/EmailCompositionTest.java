...
class EmailCompositionTest {
  ...
  void emailContent_null_html_content_throws() {
    assertThrows(NullPointerException.class, () -> new EmailContent(null, Optional.of("text")));
  }

  @Test
  void emailContent_null_text_content_throws() {
    assertThrows(
        NullPointerException.class, () -> new EmailContent(Optional.of("<p>html</p>"), null));
  }

  @Test
  void emailContent_both_empty_throws() {
    assertThrows(
        IllegalArgumentException.class, () -> new EmailContent(Optional.empty(), Optional.empty()));
  }
  ...
  void emailAttachment_null_content_type_throws() {
    assertThrows(
        NullPointerException.class, () -> new EmailAttachment("file.pdf", null, new byte[] {1}));
  }

  @Test
  void emailAttachment_null_data_throws() {
    assertThrows(
        NullPointerException.class, () -> new EmailAttachment("file.pdf", "application/pdf", null));
  }
  ...
  void emailAttachment_fromStream_reads_bytes() {
    ...
    EmailAttachment a =
        EmailAttachment.fromStream("file.txt", "text/plain", new ByteArrayInputStream(data));
    assertEquals(3L, a.size());
  ...
  void email_builder_valid() {
    ...
    Email email =
        Email.builder().from(FROM).to(RECIPIENT).subject(SUBJECT).content(content).build();
    ...
    assertThrows(
        ...
        () ->
            new TemplateEmailRequest(
                FROM, List.of(), List.of(), List.of(), SUBJECT, "tmpl", Map.of(), List.of(), null));
  }
